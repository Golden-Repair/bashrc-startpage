...
(function($) {
	...
	$.fn.tabcomplete = function(args, options) {
		console.log("tabcomplete with options: "+ args)
		if (this.length > 1) {
		...
		this.on("input.tabcomplete", function() {
			...
			if (self[0].selectionStart == input.length
					...
					console.log(args.suggestions);
					words = match(word, args.suggestions, options.caseSensitive);
					console.log("input: "+word+" - meatched words: "+words)
